...
class Spider(Spider):
    ...
    def searchContent(self, key, quick):
        try:
            ...
            post_data = f"searchword={urllib.parse.quote(key)}".encode('utf-8')
            headers = {
            ...
            rsp = self.fetch(url, data=post_data, headers=headers, method="POST")
            root = self.parse_html(rsp.content)
            ...
            selectors = [
                ...
                "//div[contains(@class,'stui-vodlist')]//li", 
            ...
            for item in result_items:
                try:
                    if item.tag == 'a':  # 直接a标签
                        ...
                        pic = self.get_first(link.xpath("./@data-original")) or self.get_first(link.xpath("./@src"))
                        if not pic:
                            style = self.get_first(link.xpath("./@style"))
                            pic = re.search(r"background-image:\s*url\(['\"]?(.*?)['\"]?\)", style).group(1) if style and re.search(r"background-image:\s*url\(['\"]?(.*?)['\"]?\)", style) else ""
                        remark = self.get_first(item.xpath(".//span[contains(@class,'pic-text')]/text()")) or self.get_first(item.xpath(".//span[@class='text-muted']/text()"))
            ...
            if not videos:
                for a in root.xpath("//a[contains(@href,'/vod/')]"):
                    try:
                        name = self.get_first(a.xpath("./@title")) or self.get_first(a.xpath(".//text()"))
                        pic = self.get_first(a.xpath("./@data-original"))
                        sid = self.get_first(a.xpath("./@href"))
